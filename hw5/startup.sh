...
LOCK_FILE="/var/log/hw5_startup_done"

if [ -f "$LOCK_FILE" ]; then
  echo "HW5 startup script already ran. Skipping."
  exit 0
fi
...
pip3 install --break-system-packages flask requests pymysql

# Read instance metadata attributes and export them as environment variables
METADATA="http://metadata.google.internal/computeMetadata/v1/instance/attributes"
HEADER="Metadata-Flavor: Google"

export DB_HOST=$(curl -s -H "$HEADER" "$METADATA/DB_HOST")
export DB_USER=$(curl -s -H "$HEADER" "$METADATA/DB_USER")
export DB_PASSWORD=$(curl -s -H "$HEADER" "$METADATA/DB_PASSWORD")
export DB_NAME=$(curl -s -H "$HEADER" "$METADATA/DB_NAME")
export ALERT_SERVICE_URL=$(curl -s -H "$HEADER" "$METADATA/ALERT_SERVICE_URL")
...
def read_file_from_gcs(filename):
    ...
    url = f"https://storage.googleapis.com/{BUCKET_NAME}/pages/{filename}"
    resp = requests.get(url, timeout=15)

    elapsed_ms = (time.perf_counter() - start) * 1000

    if resp.status_code == 200:
        return True, resp.text, elapsed_ms
...
python3 /server.py > /var/log/hw5_server.log 2>&1 &
